...
const SITE_DEFAULT = { phone:'', whatsapp:'', email:'', address:'', hours:'', heroImage:'', appImage:'', photoIds:[] };
...
function siteDoctorCard(d,s){
  ...
  return `<article class="st-dcard" data-q="${(unesc(d.name)+' '+(d.spec||'')+' '+unesc(d.clinicName||'')).toLowerCase().replace(/"/g,'')}">
    ...
    <div class="st-dbody"><div class="st-dname">${d.name}</div><div class="st-dspec">${d.spec||''}</div>
      ...
      <div class="st-dact"><button class="st-btn st-btn-or st-btn-sm" data-book="${d.slug}">Book Appointment</button><button class="st-icon-btn" data-copylink="${d.slug}" aria-label="Share ${d.name}'s booking link" title="Share link">${ICONS.share}</button></div></div></article>`;
}
...
function renderDirectory(){
  ...
  const tel=siteDigits(s.phone)?'tel:'+String(s.phone).replace(/[^\d+]/g,''):'', wa=siteWaLink(s.whatsapp||s.phone,'Namaste, mujhe '+APP_CONFIG.brand+' ke baare mein jaankari chahiye.');
  ...
  <header class="st-nav"><div class="st-wrap st-nav-in">
    ...
    <div class="st-nav-cta"><button class="st-btn st-btn-or st-btn-sm" data-login>${sic('lock',16)} Doctor Login</button><button class="st-burger" id="st-burger" aria-label="Menu" aria-expanded="false">${sic('menu',22)}</button></div></div>
  ...
  <section class="st-hero"><div class="st-wrap st-hero-in">
    <div><span class="st-pill"><i></i>${docs.length>0?docs.length+(docs.length===1?' doctor':' doctors')+' abhi booking le rahe hain':'Online appointment booking'}</span>
      <h1 class="st-h1">Doctor ki appointment,<span>ab bina line ke.</span></h1>
      <p class="st-lead">Apne doctor ka slot ya token chunein, bina login ke. Clinic pahunchte hi aapki baari taiyar.</p>
      <div class="st-hero-cta"><button class="st-btn st-btn-or" data-go="st-doctors">Appointment book karein</button><button class="st-btn st-btn-line" data-go="st-for-doctors">Doctor hain? Judiye</button></div>
      <div class="st-hero-pts"><span>${sic('tick',18)}Bina login</span><span>${sic('tick',18)}Live token</span><span>${sic('tick',18)}Fees clinic par</span></div></div>
    <div class="st-hero-img">${heroImg?`<img src="${heroImg}" alt="">`:SITE_HERO_SVG}</div>
  </div></section>

  <div class="st-strip"><div class="st-wrap st-strip-in">
    <div class="st-strip-help"><span class="st-ring">${sic('phone',22)}</span><div>${s.phone?`<a href="${tel}">${esc(s.phone)}</a><small>Booking mein madad ke liye call karein</small>`:`Booking mein madad chahiye?<small>Neeche contact section dekhein</small>`}</div></div>
    <div class="st-strip-pts"><span>${sic('cal',18)}Din ya token chunein</span><span>${sic('live',18)}Running token dekhein</span><span>${sic('rupee',18)}Fees clinic par</span></div>
  </div></div>

  <section class="st-sec" id="st-features"><div class="st-wrap">
    <div class="st-head"><div><h2 class="st-title">Patients ke liye, sab kuch aasaan</h2><p class="st-sub">Line mein khade hone ki jagah, ghar se apna number pakka karein.</p></div></div>
  ...
  <section class="st-sec st-sec-alt" id="st-doctors"><div class="st-wrap">
    <div class="st-head"><div><h2 class="st-title">Apna doctor chunein</h2><p class="st-sub">Doctor ka card dabakar seedha unka booking page kholein.</p></div>
      <label class="st-search">${sic('search',18)}<input type="search" id="st-q" placeholder="Doctor ya speciality dhundhein" aria-label="Search doctors"></label></div>
    <div class="st-docs-wrap">
      <div class="st-dgrid" id="st-dgrid">${docs.length?docs.map(d=>siteDoctorCard(d,s)).join(''):`<div class="st-empty">Abhi koi doctor booking ke liye available nahi hai. Thodi der baad dobara dekhein.</div>`}<div class="st-empty" id="st-none" hidden>Is naam ka koi doctor nahi mila.</div></div>
      <aside class="st-book"><h3>Jaldi book karein</h3><p>Doctor chunein aur seedha booking page par jayein.</p>
        <select id="st-sel" aria-label="Select doctor"><option value="">Doctor chunein</option>${docs.map(d=>`<option value="${d.slug}">${d.name} · ${d.spec||''}</option>`).join('')}</select>
        <button class="st-btn st-btn-or st-btn-block" id="st-go">Appointment book karein</button>
        <button class="st-btn st-btn-line st-btn-block" id="st-check">Meri appointment dekhein</button></aside>
    </div></div></section>
  ...
  <section class="st-sec" id="st-for-doctors"><div class="st-wrap st-band">
    ...
    <div><h2 class="st-title">Doctor hain? Apna booking page banwayein</h2><p class="st-sub">Phone par ya clinic ke reception par booking sambhalne ka jhanjhat khatam.</p>
      <ul class="st-blist">${dfeat.map(f=>`<li><div class="st-fic">${sic(f[0],22)}</div><div><b>${f[1]}</b><span>${f[2]}</span></div></li>`).join('')}</ul>
      <div class="st-band-cta"><button class="st-btn st-btn-or" data-login>${sic('lock',16)} Doctor Login</button>${hasContact?`<button class="st-btn st-btn-ghost" data-go="st-contact">Hamse sampark karein</button>`:''}</div></div>
  </div></section>

  <section class="st-sec st-sec-alt" id="st-how"><div class="st-wrap">
    <div class="st-head"><div><h2 class="st-title">Booking teen kadam mein</h2></div></div>
    <div class="st-steps">
      <div class="st-step"><h3>Doctor chunein</h3><p>Upar diye doctors mein se apne doctor ka card kholein, ya unka link ya QR scan karein.</p></div>
      <div class="st-step"><h3>Din aur slot chunein</h3><p>Khaali slot ya token dikhte hain. Jo book ho chuke, wo lock dikhte hain.</p></div>
      <div class="st-step"><h3>Details bharein</h3><p>Naam aur mobile number dalein. Booking confirm hote hi aapko appointment ID milti hai.</p></div>
    </div></div></section>

  <section class="st-sec" id="st-faq"><div class="st-wrap">
    <div class="st-head"><div><h2 class="st-title">Aksar poochhe jaane wale sawal</h2></div></div>
  ...
  ${hasContact?`<section class="st-sec st-sec-alt" id="st-contact"><div class="st-wrap"><div class="st-contact">
    <div class="st-ccard"><h3>Sampark karein</h3><p>Booking ya doctor ke onboarding ke baare mein poochhna ho to hum yahan hain.</p>
      ...
      ${wa?`<a class="st-btn st-btn-white" href="${wa}" target="_blank" rel="noopener">${sic('chat',18)} WhatsApp par likhein</a>`:''}</div>
    <div class="st-tcard"><h3>${s.hours?'Support timing':'Doctors ke liye'}</h3>${s.hours?`<p>${esc(s.hours)}</p>`:`<p>Apna booking page banwane ke liye hamse sampark karein ya seedha Doctor Login karein.</p>`}<div><button class="st-btn st-btn-or" data-login>${sic('lock',16)} Doctor Login</button></div></div>
  </div></div></section>`:''}

  <footer class="st-foot"><div class="st-wrap"><div class="st-foot-in">
    <div><a class="st-brand" href="#/"><img src="logo-mark.png" alt="" width="38" height="38"><span>${brand}</span></a><p>Har doctor ka apna booking page. Patient ke liye bina login, doctor ke liye bina jhanjhat.</p></div>
  ...
  </div><div class="st-copy"><span>© ${new Date().getFullYear()} ${brand}</span><span>Made for clinics in India</span></div></div></footer>
  </div>`;
  ...
  $('st-go').onclick=()=>{ const v=$('st-sel').value; if(!v){ toast('Pehle doctor chunein'); return; } location.hash='#/book/'+v; };
  $('st-check').onclick=()=>{ const v=$('st-sel').value; if(!v){ toast('Pehle doctor chunein'); return; } location.hash='#/my/'+v; };
...
function adminSlotsForDate(doc,date){
  ...
  return times.map((t,i)=>({ time:t, tokenIdx:i+1, past:(isToday && toMin(t)<=nowMin), appt:myAppts(doc).find(a=>a.date===date&&a.time===t&&a.status!=='cancelled')||null }));
}
...
function schedSlotTile(s,tokenWise){
  ...
  if(!s.appt){
    if(s.past) return `<div class="sched-slot past" title="Samay nikal gaya — lock hai"><div class="t">${label}</div><div class="plus">🔒</div></div>`;
    return `<div class="sched-slot empty" data-slot-time="${s.time}"><div class="t">${label}</div><div class="plus">+</div></div>`;
  }
  const st=migrateStatus(s.appt.status), cls=st==='completed'?'completed':st==='no_show'?'missed':'booked';
  return `<div class="sched-slot ${cls}" data-slot-time="${s.time}"><div class="t">${label}</div><div class="pn">${s.appt.patientName}</div><div class="pp">${st==='completed'?'✕ Completed':st==='no_show'?'No show':tokenLabel(s.appt.tokenNo)}</div></div>`;
}
...
async function openDocumentModal(doc,patient,source,onSaved){
  openModal(`<h3>Upload document</h3><div class="sub">${patient.name}</div><div class="field"><label>Name</label><input id="dm-name" placeholder="e.g. Blood test report"></div><div class="field"><label>File (image or PDF, max 5 MB)</label><input type="file" id="dm-file" accept="image/*,application/pdf"></div><button class="btn btn-coral btn-block" id="dm-go">Upload</button>`);
  $('dm-go').onclick=async()=>{ const f=$('dm-file').files[0]; if(!f){ toast('File chunein'); return; } if(f.size>5*1024*1024){ toast('File 5 MB se chhoti honi chahiye'); return; }
...
function openLedgerModal(doc,type,entry){
  ...
  $('lg-save').onclick=async()=>{ const amt=Number($('lg-a').value); if(!(amt>0)){ toast('Amount daalein'); return; } const c=$('lg-c').value.trim(); if(!c){ toast('Category/Title daalein'); return; }
    ...
    const fl=$('lg-f')&&$('lg-f').files[0]; if(fl){ if(fl.size>5*1024*1024){ toast('Receipt 5 MB se chhoti ho'); return; } try{ const sv=await Backend.saveFile(fl,{doctorId:doc.id}); rec.receipt={name:esc(fl.name),dataUrl:sv.dataUrl,mimeType:sv.mimeType,storagePath:sv.storagePath||null}; }catch(err){ toast(errMsg(err)); return; } }
...
async function drawBooking(doc,preview){
  ...
  if(bk.step===1){
    ...
    const grp=(l,a)=>a.length?`<div class="section-label" style="margin-top:12px;">${l}</div><div class="slot-grid">${a.map(s=>`<button class="slot ${!s.free?'taken':''}${s.completed?' completed':''}" ${!s.free?'disabled':''} title="${s.past?'Samay nikal gaya':fmt12(s.time)}" data-time="${s.time}">${slotLabel(s)}</button>`).join('')}</div>`:'';
...
function renderApptPage(){
  ...
  root.innerHTML=`<div class="pubwrap"><div class="row" style="justify-content:space-between;align-items:center;margin-bottom:10px;"><a href="#/book/${doc.slug}" class="muted" style="font-weight:600;">← ${doc.name}</a>${langBtn()}</div>
    <div class="confirm-card">${st==='cancelled'?`<div class="status-badge st-cancelled" style="font-size:14px;">Cancelled</div>`:`<div class="pay-success" style="margin-bottom:6px;"><div class="tick">${ICONS.check}</div></div><h2 style="font-size:22px;">${isNew?t('confirmed'):statusLabel(st)}</h2>`}
      ...
      ${a.date===todayStr()&&r.currentToken?`<div class="mt"><div class="muted" style="margin-bottom:6px;">${t('runningToken')}</div><div class="running-token-circle">${tokenLabel(r.currentToken)}</div></div>`:''}</div>
