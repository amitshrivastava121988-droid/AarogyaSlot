...
const APP_CONFIG = {
  firebase:{
    "apiKey": "AIzaSyB0FnmMZYpx6eTZta6Mtl-h8zp6uFCT1og",
    "authDomain": "appointmydoc-5537d.firebaseapp.com",
    "projectId": "appointmydoc-5537d",
    "storageBucket": "appointmydoc-5537d.firebasestorage.app",
    "messagingSenderId": "1083397569498",
    "appId": "1:1083397569498:web:b44c6b26ad0e22a3031ee3"
  },
...
const DemoBackend = {
  ...
  async bookAppointment(input){
    ...
    const appt=escDeep({ id:uid()+uid(), doctorId:doc.id, source:input.source||'patient', status:'booked', date:input.date, time:input.time, tokenNo:Math.max(0,...tokens)+1, fee:input.fee!=null?input.fee:doc.fee,
...
const FirebaseBackend = {
  ...
  async bookAppointment(input){
    ...
    const booked=await S.runTransaction(db, async tx=>{
      ...
      const appt={ id:apptRef.id, doctorId, source:input.source||'patient', status:'booked', date, time, slotKey:key+'_'+idx, slotIdx:idx, tokenNo:issued, fee:input.fee!=null?input.fee:doc.fee,
      ...
      tx.set(ticketRef,{apptId:apptRef.id,doctorId,date,time,slotKey:appt.slotKey,slotIdx:idx,tokenNo:issued,status:'booked',paymentStatus:'pending',patientName:p.name,patientPhone:normPhone(p.phone),bookedFor:p.bookedFor||null,fee:appt.fee,createdAt:appt.createdAt,code:codeVal});
  ...
  async rescheduleAppointment(doctorId,apptId,date,time,by){
    const S=FB.S, db=FB.db, tid=this._ticketOf(apptId); if(!tid) throw appErr('not_found','Appointment nahi mili');
    const tref=S.doc(db,'doctors',doctorId,'tickets',tid), aref=S.doc(db,'doctors',doctorId,'appointments',apptId); let result=null;
    await S.runTransaction(db, async tx=>{
...
/* ---- public directory (active doctors only; no earnings / patient counts) ---- */
function renderDirectory(){
  const root=$('root'), docs=state.doctors.filter(d=>d.active!==false&&!d.deleted&&!bookingClosedReason(d,state.platform.public));
  root.innerHTML=`
    <div id="app-directory">
      <div class="dir-header"><div class="brand-row"><div class="brand-mark">A</div><div><div class="brand-name">${APP_CONFIG.brand}</div><div class="brand-sub">हर डॉक्टर का अपना बुकिंग पेज · every doctor's own booking page</div></div></div>
        <div class="row gap8"><button class="btn btn-coral" id="dir-login">${ICONS.lock} Doctor Login</button></div></div>
      <div class="section-label" style="margin-top:8px;">Find your doctor</div>
      <div class="dir-grid">
        ${docs.length===0?`<div class="card">${emptyBox('Abhi koi doctor available nahi hai.','users')}</div>`:docs.map(d=>{ const rt=docRatingStats(d.id); return `
          <div class="dir-card">
            <div class="dir-top">${avatarHtml(d,'doc-avatar')}<div><div class="dir-name">${d.name}</div><div class="dir-spec">${d.spec} · ₹${d.fee}</div><div class="dir-rating">${rt.count>0?starsDisplay(rt.avg)+' '+rt.avg+' ('+rt.count+')':(d.clinicName||'')}</div></div></div>
            <div class="dir-actions"><button class="btn btn-coral btn-sm" style="flex:1;" data-book="${d.slug}">Book Appointment</button><button class="btn btn-ghost btn-sm" data-copylink="${d.slug}" title="Share link">${ICONS.share}</button></div>
          </div>`; }).join('')}
      </div>
      <div class="superadmin-link"><button id="sa-link">${ICONS.shield} Platform Admin</button></div>
    </div>`;
  $('dir-login').onclick=()=>{ location.hash='#/login'; }; $('sa-link').onclick=()=>{ location.hash='#/superadmin'; };
  root.querySelectorAll('[data-book]').forEach(b=>b.onclick=()=>{ location.hash='#/book/'+b.dataset.book; });
  root.querySelectorAll('[data-copylink]').forEach(b=>b.onclick=()=>{ const d=docBySlug(b.dataset.copylink); shareOrCopyLink(publicUrl(d.slug), d.name+' — Book an Appointment', '#/book/'+d.slug); });
}
...
function renderSuperadmin(){
  ...
  const titles={dashboard:'Dashboard',doctors:'Doctors',subscriptions:'Subscriptions',payments:'Platform Payments',appointments:'Appointments',patients:'Patients',reports:'Reports',settings:'Settings'};
  ...
  ({dashboard:saDashboard,doctors:saDoctors,subscriptions:saSubscriptions,payments:saPayments,appointments:saAppointments,patients:saPatients,reports:saReports,settings:saSettings})[ui.saTab](view);
}
...
async function drawBooking(doc,preview){
  ...
  if(bk.step===1){
    ...
    area.innerHTML=`<div class="card">${steps}<div class="section-label">3 · ${t('yourDetails2')}</div><div class="banner ok" style="margin-bottom:12px;"><span>${dow(bk.date)}, ${niceDate(bk.date)} · <b>${timeSlotLabel}</b></span><button class="btn btn-ghost btn-sm" id="bk-back">${t('back')}</button></div>
