  ...
  root.innerHTML = `
    <div class="modal-backdrop"><div class="modal wide">
      ...
      <div class="field" style="position:relative;"><label>Diagnosis</label>
        ...
        <input id="hn-diag" placeholder="e.g. Viral fever ya upar se chunein" autocomplete="off" class="mt"><div id="hn-diag-sugg" class="autosuggest-box" style="display:none;"></div></div>
...
function openFeeManagerModal(doc){
  ...
  root.innerHTML = `
    <div class="modal-backdrop"><div class="modal wide">
      ...
      <div class="field mt" style="border-top:1px solid var(--line,#e6e6e6); padding-top:14px;">
        ...
        <div class="muted" style="margin-top:6px;font-size:12px;">0 = har baar fee lagegi. Agar aap din set karte hain (jaise 7), to patient us window ke andar dobara aaye to fees free hogi — us din ke baad naya fee lagega.</div>
      </div>
  ...
  document.getElementById('mgr-fee-validity-save').addEventListener('click', async ()=>{
    ...
    await saveState(); root.innerHTML=''; renderAdmin(); toast('Fee validity saved');
  });
}
...
function openRecordPaymentModal(d){
  ...
  openModal(`<h3>Record payment — ${d.name}</h3><div class="sub">${upg?`${esc(upg.planName)} plan request approve ho rahi hai. `:''}Subscription renew hogi aur "Paid" mark hogi.</div>
...
function queueCardHtml(doc){
  ...
  return `<div class="qcard">
    <div class="switch" style="margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,.08);"><div><div class="t">${avail?'Doctor available today':'Doctor NOT available today'}</div><div class="d">${avail?'Band karne par patients ko booking page pr "Aaj available nahi" dikhega aur aaj ki booking band ho jayegi':(blk.kind==='range'?`Ye din "${esc(blk.label||'Blocked')}" ke ek lambe block ka hissa hai — poora hatane ke liye Schedule → Blocked dates mein jaayein`:'Patients ko booking page pr "Aaj available nahi" dikh raha hai')}</div></div><input type="checkbox" id="qc-avail" ${avail?'checked':''} ${blk&&blk.kind==='range'?'disabled':''}></div>
...
async function toggleTodayAvailability(doc,makeAvailable){
  ...
  if(makeAvailable){
    if(!blk) return;
    if(blk.kind==='range'){ toast('Ye din ek lambe block ka hissa hai — Schedule → Blocked dates se hatayein'); renderAdmin(); return; }
    doc.schedule=Object.assign(JSON.parse(JSON.stringify(sch)),{blocks:(sch.blocks||[]).filter(b=>b.id!==blk.id)});
    await saveState(); renderAdmin(); toast('Aaj ki booking phir se khul gayi');
  } else {
    ...
    else toast('Aaj ke liye "Not available" mark kar diya gaya');
  }
}
function openTodayUnavailableModal(doc){
  ...
  openModal(`<h3>Aaj ${list.length} appointment${list.length===1?'':'s'} book hain</h3><div class="sub">Doctor ko "Not available" mark kar diya gaya hai — in patients ko cancel ya reschedule karein.</div>
    ...
    <button class="btn btn-ghost btn-block mt" id="tu-later">Baad mein karunga</button>`,{wide:true});
  $('tu-later').onclick=closeModal;
  $('tu-cancel-all').onclick=async()=>{ if(!confirm('Sabhi '+list.length+' appointments cancel karein?')) return; for(const a of list){ await staffCancel(doc,a); } closeModal(); renderAdmin(); toast('Sabhi appointments cancel kar di gayi'); };
  document.querySelectorAll('[data-canc]').forEach(b=>b.onclick=async()=>{ const a=list.find(x=>x.id===b.dataset.canc); if(!confirm('Ye appointment cancel karein?')) return; const ok=await staffCancel(doc,a); if(ok){ closeModal(); openTodayUnavailableModal(doc); renderAdmin(); } });
...
function openManualAppointmentModal(doc,prefillDate,prefillTime,prefillName,prefillPhone,waitlistId){
  ...
  const recalcFee=()=>{
    if(feeTouched || !doc.feeValidityDays) return;
    const phone=$('ma-phone').value, d=$('ma-date').value;
    if(!validPhone(phone)) return;
    const f=feeForBooking(doc,phone,d,null);
    $('ma-fee').value=f; const note=$('ma-fee-note');
    if(note) note.textContent = f===0 ? `Free follow-up — patient ne ${doc.feeValidityDays} din ke andar visit ki thi` : '';
  };
  $('ma-phone').addEventListener('blur', recalcFee);
  const fill=async()=>{ const d=$('ma-date').value, usage=await Backend.getSlotUsage(doc.id,d), av=computeAvailability(doc,d,usage,{ignorePast:true}), sel=$('ma-time'), tokenWise=!!docSchedule(doc).tokenWise; const slotLabel=s=>tokenWise?tokenLabel(s.tokenIdx)+' — '+fmt12(s.time):fmt12(s.time); const opts=av.slots.map(s=>`<option value="${s.time}" ${!s.free?'disabled':''}>${slotLabel(s)}${s.free?'':' (booked)'}</option>`); sel.innerHTML=(opts.join('')||`<option value="">${av.closed?(av.label||'Is din visiting hours set nahi hain'):'No slots this day'}</option>`)+`<option value="__custom">Custom time…</option>`; if(prefillTime&&d===date0) sel.value=prefillTime; };
  ...
  $('ma-save').onclick=async()=>{
    ...
    try{
      ...
      const appt=await Backend.bookAppointment({doctorId:doc.id,date,time,staff:true,source:'manual',fee:maFee,patient:{name:esc(name),phone,age:$('ma-age').value.trim(),gender:$('ma-gender').value,reason:esc($('ma-reason').value.trim())}});
...
function openRescheduleModal(doc,appt,afterSave,byPatient){
  ...
  const draw=async()=>{
    ...
    openModal(`<h3>Reschedule</h3><div class="sub">${appt.patientName} · abhi ${niceDate(appt.date)}, ${fmt12(appt.time)}</div><div class="section-label mt">Nayi tareekh</div>
      ...
      <div class="section-label mt">Naya samay</div>${av.closed?`<div class="muted">${av.label||'Is din appointments available nahi'}</div>`:`<div class="slot-grid">${av.slots.map(s=>`<button class="slot ${!s.free?'taken':''}" ${!s.free?'disabled':''} style="${s.time===selTime?'outline:2px solid var(--coral);':''}" data-t="${s.time}">${slotLabel(s)}</button>`).join('')}</div>`}
...
function renderAdminSettings(doc){
  ...
  el.innerHTML=`<div class="card"><div class="section-label">Subscription & Billing</div><div class="kv-list">
      ...
      ${upg&&upg.status==='pending'?`<div class="banner warn mt"><span>Upgrade request pending — ${esc(upg.planName)} (${money(upg.price)}/${upg.cycle}), ${niceDate((upg.requestedAt||'').slice(0,10))} ko bheja gaya. Admin approve karega.</span></div><button class="btn btn-outline btn-sm mt" id="st-upg-cancel">Cancel request</button>`:`<button class="btn btn-coral btn-sm mt" id="st-upg">Request Plan Upgrade / Change</button>`}</div>
    ...
    <div class="card mt"><div class="section-label">Payments</div>${switchRow('st-upi','Online payment (UPI)','Patient ko UPI ID dikhegi; payment aap "Paid" mark karte hain',pm.upi&&pm.upi.enabled)}
      ...
      <div class="row gap8 mt"><button class="btn btn-outline btn-sm" id="st-fees">Manage fee types</button><span class="muted" style="font-size:12px;">${doc.feeValidityDays>0?`Fee valid ${doc.feeValidityDays} din tak (follow-up free)`:'Fee validity: off — har visit par fee lagti hai'}</span></div></div>
  ...
  const upgCancel=$('st-upg-cancel'); if(upgCancel) upgCancel.onclick=async()=>{ delete doc.upgradeRequest; await saveState(); toast('Request cancel kar diya'); renderAdmin(); };
...
function openUpgradeRequestModal(doc){
  ...
  openModal(`<h3>Request Plan Upgrade / Change</h3><div class="sub">Aapki request Platform Admin ke paas jayegi — wahi payment le kar plan activate karenge.</div>
  ...
  $('upg-send').onclick=async()=>{
    const sel=document.querySelector('input[name="upg-plan"]:checked'); if(!sel){ toast('Plan chunein'); return; }
    if(sel.value===curPlanId){ toast('Ye aapka current plan hai — koi doosra plan chunein'); return; }
    ...
    await saveState(); closeModal(); toast('Request bhej diya gaya — admin approve karega'); renderAdmin();
  };
}
...
async function drawBooking(doc,preview){
  ...
  if(bk.step===1){
    ...
    area.innerHTML=`<div class="card">${steps}${todayBanner}<div class="section-label">1 · ${t('chooseDate')}</div>${dateStripHtml(doc,bk.date)}<div id="day-msg"></div></div>`;
...
function renderApptPage(){
  ...
  root.innerHTML=`<div class="pubwrap"><div class="row" style="justify-content:space-between;align-items:center;margin-bottom:10px;"><a href="#/book/${doc.slug}" class="muted" style="font-weight:600;">← ${doc.name}</a>${langBtn()}</div>
    ...
    ${open?`<div class="card mt" style="display:grid;gap:10px;">${upi&&a.paymentStatus!=='paid'?`<a class="btn btn-outline btn-block" href="${upi}">Pay via UPI (${esc(pm.upi.upiId)})</a><div class="muted" style="text-align:center;">UPI se pay karne ke baad clinic payment confirm karegi.</div>`:''}
      ...
      <div class="muted" style="text-align:center;margin-top:10px;">Is link ko sambhal ke rakhein — ye aapki appointment kholta hai.</div>`:`<div style="text-align:center;margin-top:14px;"><a class="btn btn-coral" href="#/book/${doc.slug}">${t('bookNewAppointment')}</a></div>`}
