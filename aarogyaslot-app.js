...
function saveState(){ return saveNow().then(()=>true).catch(e=>{ console.error('Save failed', e); toast('Save nahi ho paya — '+errMsg(e)); return false; }); }
...
function renderAdminSettings(doc){
  ...
  el.innerHTML=`<div class="card"><div class="section-label">Subscription & Billing</div><div class="kv-list">
    ...
    <div class="card mt"><div class="section-label">Payments</div>${switchRow('st-upi','Online payment (UPI)','Patient sees your UPI ID; you mark payment as "Paid" yourself',pm.upi&&pm.upi.enabled)}
      ...
      <div class="row gap8 mt"><button class="btn btn-outline btn-sm" id="st-fees">Manage fee types</button><span class="muted" style="font-size:12px;">${doc.feeValidityDays>0?`Fee valid for ${doc.feeValidityDays} days (follow-up free within that window)`:'Fee validity: off — every visit is charged'}</span></div></div>
